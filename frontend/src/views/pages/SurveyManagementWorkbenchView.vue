<template>
  <StandardPage
    ...
    description="参照在线问卷平台的工作流，管理员可以在前端完成问卷类型配置、题型编排、发布批次设置和预览检查。"
  >
    <template #actions>
    ...
    <div class="survey-dashboard">
      <article class="survey-dashboard__card">
        ...
        <small>覆盖课程、教学支持与毕业跟踪</small>
      </article>
      <article class="survey-dashboard__card">
        <span>已发布</span>
        <strong>{{ summary.published }}</strong>
        <small>师生可见的在线问卷批次</small>
      </article>
      <article class="survey-dashboard__card">
        <span>草稿中</span>
        <strong>{{ summary.draft }}</strong>
        <small>待补题目或待设发布时间</small>
      </article>
      <article class="survey-dashboard__card">
        <span>累计答卷</span>
        <strong>{{ summary.responses }}</strong>
        <small>本地演示数据中的回收答卷</small>
      </article>
    </div>
    ...
    <div class="survey-workbench">
      <SectionCard title="问卷库">
        ...
        <div class="survey-library soft-scrollbar">
          ...
          <el-empty v-if="!filteredSurveys.length" description="当前筛选下暂无问卷" />
        </div>
      </SectionCard>
      ...
      <SectionCard title="设计画布" class="canvas-card">
        ...
        <div class="survey-meta-grid">
          ...
          <el-form-item label="副标题">
            <el-input v-model="editor.subtitle" placeholder="用于向填写人说明本次问卷目的" />
          </el-form-item>
          <el-form-item label="问卷类型">
            <el-select v-model="editor.surveyType">
              <el-option v-for="item in surveyTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        ...
        <el-form-item label="问卷说明">
          <el-input v-model="editor.description" type="textarea" :rows="3" placeholder="说明填写对象、用途、匿名策略和发布时间要求" />
        </el-form-item>

        <div class="tag-editor">
          <div class="tag-editor__list">
            <el-tag v-for="tag in editor.tags" :key="tag" closable @close="removeTag(tag)">{{ tag }}</el-tag>
          </div>
      ...
      <div class="right-stack">
        <SectionCard title="发布控制台">
          <div class="publish-panel">
            ...
            <div class="publish-actions">
              ...
              <el-button type="warning" plain @click="closeCurrentSurvey" :disabled="editor.status !== 'PUBLISHED'">结束回收</el-button>
            </div>
          </div>
        </SectionCard>
        ...
        <SectionCard title="题目属性">
          <div v-if="selectedQuestion" class="inspector-panel">
            ...
            <el-form-item label="最少选择">
              <el-input-number
                ...
                style="width: 100%;"
                :disabled="!['multiple'].includes(selectedQuestion.type)"
              />
            </el-form-item>
          ...
          <el-empty v-else description="点击中间题卡后在这里调整题目属性" />
        </SectionCard>
      </div>
    </div>
      ...
      <div class="preview-shell">
        ...
        <div class="preview-shell__list">
          <article v-for="question in editor.questions" :key="question.id" class="preview-question">
            ...
            <div v-if="question.type === 'text'" class="preview-input">{{ question.placeholder || '填写回答' }}</div>
...
function normalizeQuestions() {
  editor.questions = editor.questions.map((question, index) => ({
    ...question,
    sortNo: index + 1,
